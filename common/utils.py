...
class fluid_params():
    ...
    def __init__(self, species, params):
        ...
        """
        q, m, n, vx, vy, vz, p_para, p_perp, gamma_para, gamma_perp = np.rollaxis(
            species, axis=1)
        nSpecies = len(q)
        ...
        lambdaD_ = lambdaD[lambdaD > 0]
        lambdaD_tot = np.sqrt((1. / (1. / lambdaD_**2).sum()))

        self.T_para = T_para
        self.T_perp = T_perp
        self.T = T
        self.lambdaD = lambdaD
        self.lambdaD_tot = lambdaD_tot
